...
import { CANDLE_RANGES, type CandleIntervalId } from "../lib/candle-ranges";

type Props = {
  value: CandleIntervalId;
  onChange: (interval: CandleIntervalId) => void;
};

export function CandleIntervalSelector({ value, onChange }: Props) {
  return (
    <div
      className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
      role="tablist"
      aria-label="Kerzen-Intervall"
    >
